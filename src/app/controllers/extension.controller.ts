...
const extensionSchema = z.object({
    name: z.string().nonempty('Nombre is required').max(100, 'Name must be 100 characters or less'),
    format:  z.string().nonempty('Formato es requerido').max(100, 'Name must be 100 characters or less'),
    definition:  z.string().nonempty('Definición es Requerido').max(250, 'Name must be 250 characters or less')
    //  z.string().nonempty('Type is required').max(50, 'Type must be 50 characters or less'),
});

export default class ExtensionCtrl {
    static async createExtension(req: Request, res: Response) {
        try {
...
console.log(req.body)
            ...
            const extension = await Extension.create({ name, format, definition });
    ...
    static async getAllExtensions(_req: Request, res: Response) {
        try {
    ...
    static async getExtensionById(req: Request, res: Response) {
        try {
            const { id } = req.params;
    ...
    static async updateExtension(req: Request, res: Response) {
        try {
            const { id } = req.params;
    ...
    static async deleteExtension(req: Request, res: Response) {
        try {
            const { id } = req.params;
